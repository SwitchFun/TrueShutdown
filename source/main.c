...
void shutdown(){
	//call shutdown payload from FS
	Result rc = splInitialize();
    if (R_FAILED(rc)) {
		printf("SPL could not be initalized, Please message on our discord.\n");
    } else {
        FILE *f = fopen("romfs:/shutdown.bin", "rb");
        if (f == NULL) {
            printf("Failed to use Shutdown Method.\n");
...
int main(int argc, char **argv)
{
    ...
    printf("TrueShutdown by SwitchFun\n");
	printf("\n");
	printf("Press A to TrueShutdown.");
    ...
    while(appletMainLoop())
    {
        ...
        if (kDown & KEY_A) shutdown(); // break in order to return to hbmenu
